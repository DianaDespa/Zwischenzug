...
bool ChessBoard::generateValidQueenMove(int pos, bool isWhite){
...
//		ok = false;
	...
	for(int i = 0; i < available_positions.size(); i++) {
		std::cout<<available_positions[i]<<'\n';
	}
	srand(time(NULL));
...
bool ChessBoard::generateValidKnightMove(int pos, bool isWhite){
	...
	if (isWhite){
		...
		for(int i = 0; i < 8; i++){
			...
			if( final_pos * ( 63 - final_pos) >=0 && abs( ( pos%8 - final_pos) ) <= 2  ) {
				if( table->nametable[final_pos].name == EMPTY_CODE || ( ( table->nametable[final_pos].name - 96) * (123 - table->nametable[final_pos].name) )  > 0 ) {
					std::cout<<"calcul: "<<pos%8 - final_pos % 8<<" poz: "<<final_pos<<"\n";
					available_positions.push_back(final_pos);
				}
			}
		}
...
bool ChessBoard::generateValidBishopMove(int pos, bool isWhite){
	std::vector<int> available_positions;
	int final_pos;
//	bool ok = false;
	if (isWhite){
...
bool ChessBoard::randomPositionKnight(bool isWhite){
	...
	// Keeps track of all available knights, depending on the color of the engine
	// in the vector v.
	if (isWhite){
		for (unsigned int i = 0 ; i < 64 ; i ++){
			if (table->nametable[i].name == WHITE_KNIGHT_CODE){
				v.push_back(i);
				if(v.size() >= 2)
					break;
			}
		...
		for (unsigned int i = 63 ; i >= 0 ; i --){
			if (table->nametable[i].name == BLACK_KNIGHT_CODE){
				v.push_back(i);
				if(v.size() >= 2)
					break;
			}
	...
	if (v.size() > 0){
		// Checks knights for possible captures and moves. When one is found,
		// update initial_position.
		while (v.size() > 0){
			p = rand() % v.size();
			*backup = *table;
			//if (generateValidKnightAttack(v[p], isWhite)){
			if (generateValidKnightMove(v[p], isWhite)){
				if (!kingInDanger(isWhite)) {
					initial_position = v[p];
					delete backup;
					return true;
...
int main(){
	...
	c.movePiece(3,27);
	//c.movePiece(10,42);
	c.generateValidQueenMove(27, true);
	for(int j =7 ; j>=0 ; j--){ 
	for(int i = 8*j ; i<= 8*(j+1) -1  ; i ++ ){

		std::cout<<c.table->nametable[i].name<<" ";
	}
		std::cout << "\n";
	}
	return 0;
